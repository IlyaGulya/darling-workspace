...
rm -f "$launch_log" "$DSERVER_TEST_TRACE_FILE"
rm -f "$DPREFIX/.darlingserver.sock" "$DPREFIX/.init.pid"

# Start only darlingserver. This avoids shellspawn/launchd protocol failures
# that happen before the old runtime reaches the push_reply handler.
...
set +e
env DPREFIX="$DPREFIX" "$dserverdbg" pushreply-bad-pointer
rc=$?
set -e
