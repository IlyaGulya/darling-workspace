...
# Start just enough of the runtime to get a darlingserver socket. The oracle
# talks to the server directly so shell startup cannot become the RED reason.
...
set +e
env DPREFIX="$DPREFIX" DSERVER_TEST_FAULT_FILE="$DSERVER_TEST_FAULT_FILE" "$dserverdbg" ingest-missing-process
rc=$?
set -e
